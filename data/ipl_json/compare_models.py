...
# Let's rebuild the standardised feature dataset in memory first
...
models = {
    "XGBoost Default": XGBClassifier(verbosity=0, random_state=42),
    "XGBoost Simpler": XGBClassifier(n_estimators=50, max_depth=3, learning_rate=0.05, verbosity=0, random_state=42),
...
for name, model in models.items():
    ...
    print(f"{name:<25} Mean CV Accuracy: {cv_scores.mean() * 100:.2f}% (+/- {cv_scores.std() * 100:.2f}%)")
